...
# NOTE: OpenWrt 24.10.1 ships python3 split into many small opkg packages
# (python3-base, python3-light, python3-decimal, python3-urllib, ...). The
# Pager firmware's base image already has a bare python3 binary, so
# `command -v python3` alone is NOT a reliable "python3 is usable" check --
# it was true even on a factory-reset device that was missing the decimal
# and urllib stdlib modules our bin/*.py scripts import transitively
# (statistics -> fractions -> decimal; pathlib -> urllib). That gap made
# spectools_bridge.py / specpine_hud.py crash with ModuleNotFoundError on
# every first-run device, even though check_dependencies() reported nothing
# was missing. Always test-import the actual modules we need, not just the
# binary's existence.
check_dependencies() {
    ...
    if command -v python3 >/dev/null 2>&1; then
        python3 -c "import statistics, pathlib, decimal, urllib.parse" >/dev/null 2>&1 || need_python=1
    else
        need_python=1
    fi
...
Install via opkg now?")
    ...
    # python3-light pulls in python3-base; decimal/urllib are split out
    # separately and are NOT included in python3-light, so name them
    # explicitly or the ModuleNotFoundError gap above just recurs.
    if [ "$need_python" -eq 1 ]; then
        opkg install python3-light python3-decimal python3-urllib >> "$LOG_FILE" 2>&1
        if ! python3 -c "import statistics, pathlib, decimal, urllib.parse" >/dev/null 2>&1; then
            ERROR_DIALOG "python3 stdlib install failed — see $LOG_FILE"
            exit 1
        fi
    fi
