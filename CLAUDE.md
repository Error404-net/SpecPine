...
**Deploy to the Pager** (set `PAGER_HOST` / `PAGER_PASS` env vars for your own dev device — these default to the stock Hak5 values below if unset):

The Pager requires forced password auth — always pass `-o PreferredAuthentications=password -o PubkeyAuthentication=no` or SSH will hang trying keys.

```bash
SSH_OPTS="-o PreferredAuthentications=password -o PubkeyAuthentication=no"
PAGER_HOST="${PAGER_HOST:-172.16.42.1}"
PAGER_PASS="${PAGER_PASS:-hak5pineapple}"

sshpass -p "$PAGER_PASS" scp $SSH_OPTS pine-spectools.zip root@"$PAGER_HOST":/root/
sshpass -p "$PAGER_PASS" ssh $SSH_OPTS root@"$PAGER_HOST" 'cd /tmp && unzip -o /root/pine-spectools.zip && \
...
**Kill a stuck session** (pineapple SIGSTOPped / HUD frozen on device):
```bash
sshpass -p "$PAGER_PASS" ssh -o PreferredAuthentications=password -o PubkeyAuthentication=no root@"$PAGER_HOST" \
  'for p in $(ps w | grep "/pineapple/pineapple" | grep -v grep | awk "{print \$1}"); do kill -CONT "$p"; done; \
   for p in $(ps w | grep -E "payload-.*\.sh|specpine_hud" | grep -v grep | awk "{print \$1}"); do kill -9 "$p" 2>/dev/null; done'
```
